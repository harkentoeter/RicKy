...
def ghost_move():
    while True:
        ...
        angle = math.radians(random.randint(-45, 45))  # More aggressive movement range
        new_x = max(0, min(screen_width - 1, current_pos[0] + int(50 * math.cos(angle))))  # Larger steps
        new_y = max(0, min(screen_height - 1, current_pos[1] + int(50 * math.sin(angle))))
        pyautogui.moveTo(new_x, new_y, duration=0.05)  # Faster movement
        time.sleep(0.1)  # Less delay between movements
